using DaftAppleGames.Core.ValidatorAttributes;
using DaftAppleGames.Editor.Utility;
using UnityEditor;

namespace DaftAppleGames.Editor.PropertyValidators
{
        ...
        public override void ValidateProperty(SerializedProperty property)
        {
            ...
            if (property.propertyType == SerializedPropertyType.ObjectReference)
            {
                if (property.objectReferenceValue == null)
                {
                    ...
                    CustomEditorGUI.HelpBox_Layout(errorMessage, MessageType.Error, context: property.serializedObject.targetObject);
                }
            }
            else
            {
                string warning = requiredAttribute.GetType().Name + " works only on reference types";
                CustomEditorGUI.HelpBox_Layout(warning, MessageType.Warning, context: property.serializedObject.targetObject);
            }
        }
    }
}
